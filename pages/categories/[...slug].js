...
export async function getServerSideProps({ params, query }) {
  ...
  const sortedSubs=[...FAVORITE_SUBCATEGORIES.filter(s=>subs.includes(s)),...subs.filter(s=>!FAVORITE_SUBCATEGORIES.includes(s))];
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <nav className="text-sm mb-4">
        ...
        {slug.map((parte,i)=>(
          <span key={i}> › <Link href={{ pathname: `${basePath.split('/').slice(0,i+2).join('/')}`, query:{page:1} }} legacyBehavior><a className="hover:underline">{parte}</a></Link></span>
        ))}
      </nav>
            ...
            <Link key={item.name} href={{ pathname:`${basePath}/${item.slug}`, query:{page:1} }} legacyBehavior>
              <a className="block border rounded-lg overflow-hidden hover:shadow-lg transition">
                <div className="w-full h-48 relative">
                  <img src={item.imageURL} alt={item.name} loading="lazy" className="object-cover w-full h-full"/>
                </div>
                <div className="p-4"><h3 className="font-semibold text-lg">{item.name}</h3></div>
              </a>
            </Link>
          ))}
        </div>
